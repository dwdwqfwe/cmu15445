...
TEST(TxnExecutorTest, GarbageCollectionWithTainted) {  // NOLINT
  ...
  TxnMgrDbg("after txn5 + txn6 tainted1111", bustub->txn_manager_.get(), table_info, table_info->table_.get());
  WithTxn(txn5, ExecuteTxnTainted(*bustub, _var, _txn, "DELETE FROM table1 WHERE a = 11"));
  TxnMgrDbg("after txn5 + txn6 tainted22222", bustub->txn_manager_.get(), table_info, table_info->table_.get());
