...
auto LRUKNode::SetTure() -> void { is_evictable_ = true; }

auto LRUKNode::SetFalse() -> void { is_evictable_ = false; }

auto LRUKNode::GetId() -> frame_id_t { return fid_; }

auto LRUKNode::Evictable() -> bool { return is_evictable_; }

auto LRUKNode::GetRealDistance() -> size_t { return history_.back(); }

auto LRUKNode::GetList() -> std::list<size_t> & { return history_; }
    ...
    if (distance > pre_distance && evictable) {
      ...
      *frame_id = lru_node.second.GetId();
      if (distance == UINT_MAX) {
        real_distance = current_timestamp_ - lru_node.second.GetRealDistance();
      }
      res = true;
    } else if (distance == UINT_MAX && real_distance < current_timestamp_ - lru_node.second.GetRealDistance() &&
               evictable) {
      real_distance = current_timestamp_ - lru_node.second.GetRealDistance();
      pre_distance = distance;
      *frame_id = lru_node.second.GetId();
