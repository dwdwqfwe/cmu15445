...
void WindowFunctionExecutor::Init() {
  ...
  if (!order_by.empty()) {
    ...
    std::sort(tuples.begin(), tuples.end(), CmpSort(&child_executor_->GetOutputSchema(), order_by));
  }
  // 用于存储窗口函数的key
  ...
  for (const auto &this_tuple : tuples) {
    ...
    for (uint32_t i = 0; i < cloumn_size; ++i) {
      ...
      if (is_function_expr[i]) {
        // 获取窗口函数的key
        auto agg_key = MakeAggregateKey(&this_tuple, partition_by[i]);
        // 如果是rank函数，则需要特殊处理
        ...
        auto agg_val = MakeAggregateValue(&this_tuple, window_exprs[i]);
...
auto WindowFunctionExecutor::MakeAggregateKey(const Tuple *tuple, std::vector<AbstractExpressionRef> &express)
    -> AggregateKey {
  std::vector<Value> keys;
  keys.reserve(express.size());
  for (auto &exp : express) {
    keys.emplace_back(exp->Evaluate(tuple, GetOutputSchema()));
  }
  return {keys};
}
auto WindowFunctionExecutor::MakeAggregateValue(const Tuple *tuple, AbstractExpressionRef &express) -> Value {
  Value keys;
  keys = (express->Evaluate(tuple, GetOutputSchema()));
  return {keys};
}
}  // namespace bustub
