...
class Trie {
 ...
 public:
  ...
  auto RemoveDfs(std::string_view &key, const std::shared_ptr<TrieNode> &node, size_t pos) const -> bool;
  ...
  auto PutDfs(const std::shared_ptr<TrieNode> &node, std::string_view key, size_t pos, T value) const -> void;
};
