...
struct VersionUndoLink {
  ...
  timestamp_t base_ts_{INVALID_TS};
...
class TransactionManager {
 public:
  ...
  void SetTxnTained(Transaction *txn);
  ...
  std::unordered_map<RID, timestamp_t> hash_new_time_;
  std::unordered_map<txn_id_t, uint32_t> undolog_counts_;
